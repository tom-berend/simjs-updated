...
export class Sim extends Model {
    ...
    intervalTimer: any    // an id number for the timer, different for Node and for browser
    ...
    simInterval = async () => {
        ...
        if (Model.queue.length == 0) {
            ...
            Model.queue.sort((prev: Request, curr: Request) => prev.deliverAt - curr.deliverAt)
    ...
    *
    */
    oldsendMessage(callback: Function, delay: number, toEntity: Entity) {
    ...
    async sendMessage(callback:Function,delay: number=0) {
...
export class Facility extends Sim {
    ...
    //     while (this.free > 0 && !this.facilityQueue.length ==0) {
...
export class BufferPool extends Sim {           // 'Buffer' conflicts with node.js usage
    ...
    constructor(name: string, capacity: number, initial: number = 0) {
        super(name);
...
export class Entity extends Model {
    ...
    oldsetTimer(duration: number): Request {
        this.debug(3, `SetTimer(): Creating a new Request '${this.name}'`)
    ...
    getBuffer(buffer: BufferPool, amount: number): Request {
