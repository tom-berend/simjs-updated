...
exports.argCheck = exports.Entity = exports.Event = exports.Store = exports.Buffer = exports.Facility = exports.Sim = void 0;
...
class Sim {
    constructor() {
        this.simTime = 0;
        this.entities = [];
        this.queue = new queues_js_1.PQueue();
        this.endTime = 0;
        this.entityId = 1;
    }
    time() {
...
exports.Sim = Sim;
class Facility extends model_js_1.Model {
    constructor(name, discipline, servers, maxqlen) {
        ...
        switch (discipline) {
            case Facility.LCFS:
                ...
                this.queue = new queues_js_1.Queue();
                break;
            case Facility.PS:
    ...
    useFCFSSchedule(timestamp) {
        ...
        while (this.free > 0 && !this.queue.empty()) {
            ...
            const newro = new request_js_1.Request(this, timestamp, timestamp + ro.duration);
    ...
    useProcessorSharing(duration, ro) {
        argCheck(arguments, 2, 2, null, request_js_1.Request);
        ro.duration = duration;
        ro.cancelRenegeClauses();
        this.stats.enter(ro.entity.time());
    ...
    useProcessorSharingSchedule(ro, isAdded) {
        ...
        for (let i = 0; i < size; i++) {
            ...
            const newev = new request_js_1.Request(this, current, current + (ev.deliverAt - current) * multiplier);
            newev.ro = ev.ro;
            newev.source = this;
        ...
        if (isAdded) {
            const newev = new request_js_1.Request(this, current, current + ro.duration * (size + 1));
            newev.ro = ro;
            newev.source = this;
            newev.deliver = this.useProcessorSharingCallback;
            newQueue.push(newev);
...
class Store extends model_js_1.Model {
...
exports.Store = Store;
class Event extends model_js_1.Model {
    constructor(name) {
        super(name);
...
exports.Event = Event;
class Entity extends model_js_1.Model {
    ...
    setTimer(duration) {
        argCheck(arguments, 1, 1);
        const ro = new request_js_1.Request(this, this.sim.time(), this.sim.time() + duration);
        this.sim.queue.insert(ro);
        return ro;
    }
    waitEvent(event) {
        argCheck(arguments, 1, 1, Event);
        const ro = new request_js_1.Request(this, this.sim.time(), 0);
        ro.source = event;
    ...
    send(message, delay, entities) {
        argCheck(arguments, 2, 3);
        const ro = new request_js_1.Request(this.sim, this.time(), this.time() + delay);
        ro.source = this;
        ro.msg = message;
        ro.data = entities;
